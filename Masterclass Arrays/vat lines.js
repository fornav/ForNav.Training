...
vatLines.map(vatLine => vatLine.lineAmount).reduce((partialSum, a) => partialSum + a, 0)

vatLines.map(vatLine => CurrReport.DotNetFormat(vatLine.vatAmount, '#,0.00')).join('\n')
vatLines.map(vatLine => vatLine.vatAmount).reduce((partialSum, a) => partialSum + a, 0)

vatLines.map(vatLine => CurrReport.DotNetFormat(vatLine.vatBase, '#,0.00')).join('\n')
vatLines.map(vatLine => vatLine.vatBase).reduce((partialSum, a) => partialSum + a, 0)